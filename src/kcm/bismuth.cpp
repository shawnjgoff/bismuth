...
Bismuth::Config *BismuthSettings::config() const { return m_config; }
...
void BismuthSettings::reloadKWinScript() const {
  OrgKdeKwinScriptingInterface kwinInterface(QStringLiteral("org.kde.KWin"),
                                             QStringLiteral("/Scripting"),
                                             QDBusConnection::sessionBus());

  // Unload Bismuth, so that it can reload its configuration
  kwinInterface.unloadScript(QStringLiteral("bismuth"))
      .waitForFinished(); // Sync call
